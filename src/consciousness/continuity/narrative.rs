...
impl NarrativeStore {
    ...
    pub fn append(&mut self, episode: Episode) {
        if let Some(existing) = self.find_similar(&episode) {
            ...
            self.episodes[idx].significance =
                (self.episodes[idx].significance + episode.significance) / 2.0;
            ...
            {
                self.episodes[idx].valence_score = Some((existing_v + new_v) / 2.0);
    ...
    pub fn high_valence_episodes(&self, threshold: f64) -> Vec<&Episode> {
        self.episodes
            .iter()
            .filter(|e| e.valence_score.is_some_and(|v| v.abs() >= threshold))
            .collect()
    }

    pub fn to_prompt_context(&self, max_entries: usize) -> String {
        if self.episodes.is_empty() {
            return String::new();
        }
        let recent: Vec<&Episode> = self
            .episodes
            .iter()
            .rev()
        ...
        let mut lines = vec!["[Session Narrative]".to_string()];
